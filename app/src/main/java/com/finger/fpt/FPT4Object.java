 ...
 * 该类目前主要用于解析和调试有问题的fpt数据文件使用，一般情况请使用FPTFile
 */
@Deprecated
public final class FPT4Object {
	...
	private List<FPTlt_tlMatchData> lt_tlMatchDataList = new ArrayList<FPTlt_tlMatchData>();//指纹正查和倒查比中信息
	private List<FPTttMatchData> ttMatchDataList = new ArrayList<FPTttMatchData>();//指纹查重比中信息
	private List<FPTllMatchData> llMatchDataList = new ArrayList<FPTllMatchData>();//指纹串查比中信息
	private List<FPTlpMatchRequestData> lpMatchRequestDataList = new ArrayList<FPTlpMatchRequestData>();//现场指纹查询请求信息
	private List<FPTtpMatchRequestData> tpMatchRequestDataList = new ArrayList<FPTtpMatchRequestData>();//十指指纹查询请求信息
	private List<FPTltMatchCandidateData> ltMatchCandidateDataList = new ArrayList<FPTltMatchCandidateData>();//正查比对结果候选信息
	private List<FPTtlMatchCandidateData> tlMatchCandidateDataList = new ArrayList<FPTtlMatchCandidateData>();//倒查比对结果候选信息
	private List<FPTttMatchCandidateData> ttMatchCandidateDataList = new ArrayList<FPTttMatchCandidateData>();//倒查比对结果候选信息
	private List<FPTllMatchCandidateData> llMatchCandidateDataList = new ArrayList<FPTllMatchCandidateData>();//倒查比对结果候选信息
	 ...
	 * @throws Exception
	 */
	public static FPT4Object parseOfInputStream(InputStream in) throws Exception{
		FPT4Object fptObject = new FPT4Object();
...
//					fptObject.customDataList.add(FPTCustomData.parseFromInputStream(in, length_));
	 ...
	 * @throws Exception
	 */
	public static FPT4Object parseOfFile(File file) throws Exception{
...
//		private String version;//版本编号
	...
	public static class FPTtpData {
		...
		private List<FingerData> fingerDataList = new ArrayList<FingerData>();//指纹数据列表
...
//			}while(in.read(end) != -1 ? "1d".equals(ByteUtil.bytes2HexString(end)) : false);//如果是GS组分隔符 继续读取
	...
	public static class FPTlpData {
		...
		private List<FingerData> fingerDataList = new ArrayList<FingerData>();//指纹数据
...
//			byte[] dataType = new byte[2];//记录类型04
			...
			byte[] caseId = new byte[23];//指纹编号
			byte[] seqNo = new byte[2];//现场指纹序号
			byte[] personId = new byte[23];//捺印卡号
	...
	public static class FPTttMatchData{
		...
		private int index;//序号
		private String systemType;//系统类型
		private String personId1;//捺印卡号1
		private String personId2;//捺印卡号2
		private String matchUnitCode;//比对单位代码
		private String matchName;//比对单位名称
		private String matcher;//比对人姓名
		private String matchDate;//比对日期
...
//			byte[] dataType = new byte[2];//记录类型04
			byte[] index = new byte[6];//序号
			byte[] systemType = new byte[4];//系统类型
			byte[] personId1 = new byte[23];//捺印卡号1
			byte[] personId2 = new byte[23];//捺印卡号2
			byte[] matchUnitCode = new byte[12];//比对单位代码
			byte[] matchName = new byte[70];//比对单位名称
			byte[] matcher = new byte[30];//比对人姓名
			byte[] matchDate = new byte[8];//比对日期
	...
	public static class FPTllMatchData{
		...
		private int index;//序号
		private String systemType;//系统类型
		private String caseId1;//指纹编号1
		private String seqNo1;//现场指纹序号1
		private String caseId2;//指纹编号2
...
//			byte[] dataType = new byte[2];//记录类型06
			byte[] index = new byte[6];//序号
			byte[] systemType = new byte[4];//系统类型
			byte[] caseId1 = new byte[23];//指纹编号1
			byte[] seqNo1 = new byte[2];//指纹序号1
			byte[] caseId2 = new byte[23];//指纹编号2
...
//			byte[] dataType = new byte[2];//记录类型08
	...
	public static class FPTltMatchCandidateData{
		...
		private String caseId;//送检指纹指纹编号
...
//			byte[] dataType = new byte[2];//记录类型09
	...
	public static class FPTtlMatchCandidateData{
		...
		private String matchUnitCode;//比对单位代码
		private String matchFinshDate;//比对完成时间
		private String personId;//送检指纹捺印卡号
...
//			byte[] dataType = new byte[2];//记录类型10
	...
	public static class FPTttMatchCandidateData{
		...
		private String matchUnitCode;//比对单位代码
		private String matchFinshDate;//比对完成时间
		private String submitPersonId;//送检指纹捺印卡号
...
//			byte[] dataType = new byte[2];//记录类型04
	...
	private static String bytes2String(byte[] bs){
		...
		try {
			return out.toString("GB2312").trim();
